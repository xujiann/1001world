...
function dailyBoardHTML9() {
  ...
  const nf9 = nextFest9();
  return `<div class="qBox" style="border:1px solid rgba(140,220,180,.4);background:rgba(50,140,100,.08)"><div class="qTitle"><span>📋 今日岛报</span><span>${left9 ? '剩 ' + left9 + ' 件' : '✅ 全清'}</span></div>${it9.map(([ic9, nm9, ok9, hint9]) => `<div class="qRow${ok9 ? ' ok' : ''}"><span>${ok9 ? '✅' : ic9} ${nm9}</span><span class="qn">${ok9 ? '已办' : hint9}</span></div>`).join('')}${nf9 ? `<div style="font-size:11.5px;color:#8ea6b8;padding:5px 2px 0">🗓️ ${nf9.d === 0 ? '今天就是 ' + nf9.emoji + nf9.name + '!' : '距 ' + nf9.emoji + nf9.name + ' 还有 ' + nf9.d + ' 天'}</div>` : ''}</div>`;
}
